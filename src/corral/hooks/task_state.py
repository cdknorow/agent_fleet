"""CLI entry point for the PostToolUse hook that syncs agent tasks to the Corral dashboard.

This is the Corral-side orchestration: read hook JSON from stdin, use the
...
from corral.hooks.utils import cache_dir, corral_api, debug_log, resolve_agent_type, resolve_session_id
...
def main():
    ...
    if task_event["action"] == "create":
        ...
        corral_api(base, "POST", f"/api/sessions/live/{agent_name}/tasks", payload)
        ...
        if status in ("completed", "in_progress"):
            ...
            if title:
                ...
                tasks = corral_api(base, "GET", f"/api/sessions/live/{agent_name}/tasks{qs}")
                ...
                if tasks:
                    for t in tasks:
                        if t.get("title") == title and t.get("completed") != 1:
                            ...
                            corral_api(base, "PATCH", f"/api/sessions/live/{agent_name}/tasks/{t['id']}", {"completed": completed_value})
                            break
